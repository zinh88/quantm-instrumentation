 ...
 */
export class QuantmHttpInstrumentation extends InstrumentationBase<Http> {
  ...
  constructor(config?: HttpInstrumentationConfig) {
    ...
    console.log(config?.quantmConfig?.changesetID);
    console.log(config?.quantmConfig?.workloadID);
  }
  ...
  private _incomingRequestFunction(
    ...
    return function incomingRequest(
      ...
      if(baggage) {
        console.log("baggage found");
      } else {
        console.log("no baggage found");
      }
  ...
  private _outgoingRequestFunction(
    ...
    return function outgoingRequest(
      ...
      const csid : string = instrumentation._getConfig().quantmConfig?.changesetID ?? "none";
      const wid : string = instrumentation._getConfig().quantmConfig?.workloadID ?? "none";
      const baggage = propagation.createBaggage({
        "qtm-csid": { value: csid },
        "qtm-wid": {value: wid}
      });

      const reqContext = propagation.setBaggage(requestContext, baggage);

      propagation.inject(reqContext, optionsParsed.headers);

      return context.with(reqContext, () => {
        /*
